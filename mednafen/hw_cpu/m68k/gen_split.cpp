...
// g++ -std=gnu++14 -Wall -O2 -o gen_split gen_split.cpp
// ./gen_split m68k_instr_split%d.inc 2
...
int main(int argc, char *argv[])
{
 if(argc < 2 || argc > 3)
 {
  printf("Usage: %s <output filename pattern> [split count]\n", argv[0]);
  return 1;
 }
 char *output_pattern = argv[1];
 if(strlen(output_pattern) == 0)
 {
  printf("Error: need a string for output filename pattern!\n");
  return 1;
 }
 int split_count = 1;
 if(argc == 3)
  split_count = std::atoi(argv[2]);
 if(split_count < 1 || split_count > 16)
 {
  printf("Error: invalid split count! (%d)\n", split_count);
  return 1;
 }
 unsigned sections_per_file = std::ceil(16.0 / split_count);

 std::vector<std::pair<std::string, std::vector<unsigned>>> bm[16];
 
 for(unsigned i = 0; i < 65536; i++)
 {
  std::string body;
  unsigned section = i >> 12;
  ...
  if(body.size() > 0)
  {
   if(bm[section].size() && bm[section].back().first == body)
    bm[section].back().second.push_back(i);
   else
    bm[section].push_back({body, {i}});
  }
 }
 ...
 {
  if(i % sections_per_file == 0)
  {
   if(f)
    fclose(f);
   f = fopen(s(output_pattern, i / sections_per_file).c_str(), "w");
  }
  for(auto const& bme : bm[i])
  {
   for(auto const& ve : bme.second)
   {
    fprintf(f, "case 0x%04x:\n", ve % (0x1000 * sections_per_file));
   }
