 ...
 * in the code segment to per-slot symbols.  Only the emulator thread
 * compiles slots, so the fd and index counter are unserialized; atexit
 * fires after that thread exits.
 */
