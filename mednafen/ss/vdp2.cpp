 ...
 while(HCounter >= HTimings[HRes & 1][HPhase])
 {
  ...
  if(HPhase == HPHASE_ACTIVE)
  {
   ...
   if(VPhase == VPHASE_ACTIVE)
   {
    ...
    lib->vdp1_hires8 = VDP1::GetLine(VCounter, lib->vdp1_line, lib->vdp1_mesh_line, &lib->vdp1_line_src, &lib->vdp1_mesh_line_src, (HRes & 1) ? 352 : 320, (int32)RotParams[0].XstAccum >> 1, (int32)RotParams[0].YstAccum >> 1, (int32)RotParams[0].DX >> 1, (int32)RotParams[0].DY >> 1); // Always call, has side effects.
